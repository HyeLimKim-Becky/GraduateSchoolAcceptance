...
hist(rawdata$University.Rating,main='????????????',xlab='University rating',col='black')
hist(rawdata$SOP,main='???????????????',xlab='sop',col='yellow')
hist(rawdata$LOR,main='????????? ??????',xlab='lor',col='green')
hist(rawdata$CGPA,main='?????? ??????',xlab='cgpa',col='blue')
hist(rawdata$Research,main='?????? ??????',xlab='research',col='navy')
hist(rawdata$Chance.of.Admit,main='????????? ????????????',xlab='Admission chance',col='purple')
...
boxplot(rawdata$University.Rating,main='????????????',xlab='University rating',col='black')
boxplot(rawdata$SOP,main='???????????????',xlab='sop',col='yellow')
boxplot(rawdata$LOR,main='????????? ??????',xlab='lor',col='green')
boxplot(rawdata$CGPA,main='?????? ??????',xlab='cgpa',col='blue')
boxplot(rawdata$Research,main='?????? ??????',xlab='research',col='navy')
boxplot(rawdata$Chance.of.Admit,main='????????? ????????????',xlab='Admission chance',col='purple')

# Pie chart
par(mfrow=c(1,2),mar=c(1,1,1,1))
pie(Uni_Rating_Table,main='????????? ??????',radius = 1)
pie(Research_Table,main='?????? ??????',radius=1)
